...
DOCK_COMPOSE_TEST_FILES = -f docker-compose.yaml -f docker-compose.dev.yaml -f docker-compose.test.yaml
...
test:
	$(COMPOSE_CMD) $(DOCK_COMPOSE_TEST_FILES) build
	$(COMPOSE_CMD) $(DOCK_COMPOSE_TEST_FILES) up
